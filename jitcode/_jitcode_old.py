...
def convert_to_required_symbols(dynvars, f_sym, helpers=[]):
	"""
	...
	f : iterable of SymPy expressions or one-dimensional SymPy matrix
	...
	Returns
	-------
	converted_f : one-dimensional SymPy matrix
		same as the respective input for `jitcode`
	converted_helpers : list of pairs, each containing a SymPy symbol and a SymPy expression
		same as the respective input for `jitcode`
	"""
	
	t, y = provide_basic_symbols()
	substitutions = [(dynvar, y(i)) for i,dynvar in enumerate(dynvars)]
	f = sympy.Matrix(f_sym).subs(substitutions)
	helpers = [(helper[0], helper[1].subs(substitutions)) for helper in helpers]
	return (f, helpers)

def ode_from_module_file(location):
	"""
	loads functions from a module file generated by JIC-ODE (see `save_compiled`).
...
def _is_C(function):
	return type(function) == BuiltinFunctionType
...
def _jac_from_f_with_helpers(f, helpers):
	t,y = provide_basic_symbols()
	n = len(f)
	...
	for i in range(n):
		row = []
		for j in range(n):
			entry = sympy.diff( f[i], y(j) )
			for helper in dependent_helpers[j]:
				entry += sympy.diff(f[i],helper[0]) * helper[1]
			row.append(entry)
		jac.append(row)
	
	return sympy.Matrix(jac)

class jitcode(ode):
	"""
	Parameters
	----------
	f_sym : iterable of SymPy expressions or one-dimensional SymPy matrix
	...
	wants_jacobian : boolean
		Tell JIC-ODE to calculate and compile the Jacobian. For vanilla use, you do not need to bother about this as this is automatically set to `True` if the selected method of integration desires the Jacobian. However, it is sometimes useful if you want to manually apply some code-generation steps (e.g., to apply some tweaks).
	"""
	...
	def __init__(self, f_sym, helpers=None, wants_jacobian=False):
		self.f_sym = sympy.Matrix(f_sym).doit()
		#self.f_sym = (entry.doit() for entry in f_sym)
		self.f = None
	...
	def generate_jac_sym(self, simplify=True):
		...
		self.jac_sym = _jac_from_f_with_helpers(self.f_sym, self.helpers)
		if simplify:
			self.jac_sym.simplify(ratio=1.0)
	...
	def generate_f_C(self, simplify=True, do_cse=True, chunk_size=100):
		...
		if simplify:
			self.f_sym = sympy.Matrix(sympy.simplify(self.f_sym,ratio=1))
		...
		if do_cse:
			...
			more_helpers = _cse[0]
			f_sym_reduced = _cse[1][0]
		else:
			more_helpers = []
			f_sym_reduced = self.f_sym
		...
		render_and_write_code(
			(set_dy(i,entry) for i,entry in enumerate(f_sym_reduced)),
			more_helpers,
			self._tmpfile(),
			"f",
			{"set_dy":"set_dy", "y":"y"},
	...
	def generate_jac_C(self, do_cse=True, chunk_size=100, sparse=0.5):
		"""
		...
		sparse : boolean or number
			Whether a sparse Jacobian should be assumed for optimisation.
			If `sparse` is a number, sparseness is assumed if the ratio of non-zero entries is smaller than `sparse`.
			Note that this does not mean that the Jacobian is stored, parsed or handled as a sparse matrix.
		"""
		
		self._generate_jac_sym()
		jac_sym_wc = sympy.Matrix(self.jac_sym)
		
		if type(sparse) == bool:
			self.sparse_jac = sparse
		else:
			self.sparse_jac = non_zero_ratio(jac_sym_wc) < sparse
		...
		render_and_write_code(
			(
				set_dfdy(i,j,entry)
				for i,line in enumerate(jac_sym_wc.tolist())
	...
	def compile_C(
		self,
		extra_compile_args = ["-O3","-march=native","-mtune=native","-Wno-unknown-pragmas"],
		verbose = False,
		modulename = None
		):
		"""
	...
	def generate_f_lambda(self, simplify=True):
		...
		n = len(self.f_sym)
		t,y = provide_basic_symbols()
		Y = sympy.symarray("Y", n)
		
		substitutions = self.helpers[::-1] + [(y(i),Y[i]) for i in range(n)]
		f_sym_subsed = self.f_sym.subs(substitutions)
		if simplify:
			f_sym_subsed.simplify(ratio=1.0)
		F = sympy.lambdify([t]+[Yentry for Yentry in Y], f_sym_subsed)
	...
	def generate_jac_lambda(self):
		...
		substitutions = self.helpers[::-1] + [(y(i),Y[i]) for i in range(n)]
		jac_subsed = sympy.Matrix(self.jac_sym).subs(substitutions)
	...
	def set_integrator(self, name, **integrator_params):
		...
		if name == 'zvode':
			raise NotImplementedError("JIC-ODE does not natively support complex numbers yet.")
	...
	def set_f_params(self, *args):
		raise NotImplementedError("JIC-ODE does not support passing parameters to the derivative yet.")
	
	def set_jac_params(self, *args):
		raise NotImplementedError("JIC-ODE does not support passing parameters to the Jacobian yet.")
...
class jitcode_lyap(jitcode):
	...
	def __init__(self, f_sym, helpers=None, wants_jacobian=False, nlyap=-1):
		
		f_lyap = copy_object(f_sym)
		n = self._n = len(f_lyap)
		self._nlyap = n if (nlyap<0 or nlyap>n) else nlyap
		
		t,y = provide_basic_symbols()
		
		helpers = _sort_helpers(_sympify_helpers(helpers or []))
		jac = _jac_from_f_with_helpers(f_sym, helpers)
		
		for i in range(nlyap):
			for j in range(n):
				entry = sum( jac[j,k] * y(k+(i+1)*n) for k in range(n) )
				f_lyap.append( sympy.simplify(entry, ratio=1.0) )
	...
	def set_initial_value(self, y, t=0.0):
		new_y = [y]
		for i in range(self._nlyap):
			new_y += [random_direction(self._n)]
	...
	def integrate(self, *args, **kwargs):
		...
		n = self._n
		tangent_vectors = [ self._y[(i+1)*n:(i+2)*n] for i in range(self._nlyap) ]
