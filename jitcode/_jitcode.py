...
class jitcode(ode,jitcxde):
	...
	def compile_C(
		...
		omp : boolean or pair of list of strings
			Whether the C code shall be compiled for multiprocessing (using OpenMP). Note that this will only work if you have a properly defined chunk_size. If `True`, the required additional compiler and linker arguments will be chosen automatically. If you do not want this or it doesn’t work, specify them as a pair of lists of strings: The first element are the compiler arguments, the second one are the linker arguments.
		"""
