...
Estimates for the Lyapunov vectors are returned as well. There also is a class (`jitcode_restricted_lyap`) that allows to calculate Lyapunov exponents for the dynamics transversal to some manifold (such as a synchronisation manifold).
